...
class QueryTest extends TestCase
{
    ...
    public function testConditionsSimple()
    {
        $query = Query::create()
            ->setEscapeFunction([$this->getPDOInstance(), 'quote'])
            ->select(['*'])
    ...
    public function testConditionsOperators()
    {
        $query = Query::create()
            ->setEscapeFunction([$this->getPDOInstance(), 'quote'])
            ->select(['*'])
        ...
        $this->assertEquals(
            "SELECT * FROM `the_table` WHERE (`name` IS NOT NULL) AND (`lastname` != NULL) AND (`age` >= 15) AND (`city` = 'Quebec') LIMIT 0,100",
            (string)$query
        );
    }
